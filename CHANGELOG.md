...
This is the **Grolt** fork — a rebranded edition of Clawdbot/OpenClaw focused on xAI Grok models. Grok + Molt = Grolt.
All internal references to "clawdbot"/"Clawdbot"/"CLAWDBOT" have been renamed to "moltx"/"MoltX"/"MOLTX", and the project is branded as **Grolt**.
...
- CLI: add `moltx system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.clawd.bot/cli/system
- CLI: add live auth probes to `moltx models status` for per-profile verification. (commit 40181afde) https://docs.clawd.bot/cli/models
- CLI: restart the gateway by default after `moltx update`; add `--no-restart` to skip it. (commit 2c85b1b40)
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `moltx models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Doctor: honor MOLTX_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- CLI: flatten node service commands under `moltx node` and remove `service node` docs.
- CLI: move gateway service commands under `moltx gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `moltx update wizard` for interactive channel selection and restart prompts. https://docs.clawd.bot/cli/update
...
- ACP: add `moltx acp` for IDE integrations. https://docs.clawd.bot/cli/acp
- ACP: add `moltx acp client` interactive harness for debugging. https://docs.clawd.bot/cli/acp
...
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `moltx update`.
- Plugins: share npm plugin update logic between `moltx update` and `moltx plugins update`.
...
- Exec approvals: migrate approvals to `~/.moltx/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.clawd.bot/tools/exec-approvals
- Nodes: add headless node host (`moltx node start`) for `system.run`/`system.which`. https://docs.clawd.bot/cli/node
...
- Build: use tsgo for dev/watch builds by default (opt out with `MOLTX_TS_COMPILER=tsc`).
...
### Breaking
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `moltx doctor --fix` to repair, then update plugins (`moltx plugins update`) if you use any.

### Fixes
- Discovery: shorten Bonjour DNS-SD service type to `_moltx-gw._tcp` and update discovery clients/docs.
...
- CLI: keep `moltx logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `moltx memory status`.
...
- Memory: probe sqlite-vec availability in `moltx memory status`.
...
- Plugins: add Zalo Personal plugin (`@moltx/zalouser`) and unify channel directory for plugins. (#1032) - thanks @suminhthanh. https://docs.clawd.bot/plugins/zalouser
...
### Breaking
- **BREAKING:** `moltx message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) - thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) - thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `moltx hooks` is now `moltx webhooks`; hooks live under `moltx hooks`. https://docs.clawd.bot/cli/webhooks
- **BREAKING:** `moltx plugins install <path>` now copies into `~/.moltx/extensions` (use `--link` to keep path-based loading).
...
- Directory: unify `moltx directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `moltx.hooks` manifests and `moltx hooks install/update`.
...
- CLI: speed up `moltx sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
### Highlights
- Plugins: add provider auth registry + `moltx models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@moltx/msteams` via `moltx plugins install @moltx/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) - thanks @thewilloftheshadow.

### Changes
- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) - thanks @thewilloftheshadow.
- CLI: set process titles to `moltx-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `moltx security audit`.
- Apps: store node auth tokens encrypted (Keychain/SecurePrefs).
- Daemon: share profile/state-dir resolution across service helpers and honor `MOLTX_STATE_DIR` for Windows task scripts.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `moltx models status`, and update docs.
- CLI: add `--json` output for `moltx daemon` lifecycle/install commands.
...
- Fix: make `moltx update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) - thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) - thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `moltx configure`, even if no other sections are chosen.
...
- Browser control: Chrome extension relay takeover mode + remote browser control via `moltx browser serve`.
- Plugins: channel plugins (gateway HTTP hooks) + Zalo plugin + onboarding install flow. (#854) - thanks @longmaba.
- Security: expanded `moltx security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.

### Changes
- Docs: clarify per-agent auth stores, sandboxed skill binaries, and elevated semantics.
- Docs: add FAQ entries for missing provider auth after adding agents and Gemini thinking signature errors.
- Agents: add optional auth-profile copy prompt on `agents add` and improve auth error messaging.
- Security: expand `moltx security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `moltx browser extension install/path` and remote browser control via `moltx browser serve` + `browser.controlToken`.
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.moltx-dev`).
...
- Memory: new `moltx memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.moltx/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `moltx dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) - thanks @jeffersonwarrior.
...
### Installer
- Install: run `moltx doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Onboarding/Auth: honor `MOLTX_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) - thanks @roshanasingh4.
...
- Gateway/CLI: honor `MOLTX_LAUNCHD_LABEL` / `MOLTX_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `moltx doctor --non-interactive` during updates. (#781) - thanks @ronyrus.
...
- Plugins: add `moltx plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `moltx` CLI when missing in local mode; install via `clawd.bot/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer UX: support `MOLTX_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `moltx.extensions`; load plugin packages from config dirs; extract archives without system tar.
- Config: expand `~` in `MOLTX_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) - thanks @pasogott.
...
### Highlights
- CLI: `moltx status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `moltx update` (safe-ish git checkout update) + `--update` shorthand. (#673) - thanks @fm1randa.
...
- CLI: add `moltx reset` and `moltx uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `moltx gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `moltx status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `moltx sandbox explain` (effective policy inspector + fix-it keys); improve "sandbox jail" tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: respect `MOLTX_STATE_DIR` for node pairing + voice wake settings storage. (#664) - thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`MOLTX_LIVE_GATEWAY_IMAGE_PROBE=1`).
- CLI: `moltx sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize user-facing "ClawdBot"/"MOLTX" → "MoltX" (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `moltx update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/moltx`. (#660)
...
### Breaking
- CLI: `moltx message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `moltx models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.moltx/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- Gateway/Daemon/Doctor: atomic config writes; repair gateway service entrypoint + install switches; non-interactive legacy migrations; systemd unit alignment + KillMode=process; node bridge keepalive/pings; Launch at Login persistence; bundle MoltXKit resources + Swift 6.2 compat dylib; relay version check + remove smoke test; regen Swift GatewayModels + keep agent provider string; cron jobId alias + channel alias migration + main session key normalization; heartbeat Telegram accountId resolution; avoid WhatsApp fallback for internal runs; gateway listener error wording; serveBaseUrl param; honor gateway --dev; fix wide-area discovery updates; align agents.defaults schema; provider account metadata in daemon status; refresh Carbon patch for gateway fixes; restore doctor prompter initialValue handling.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `moltx pairing list <provider>` + `moltx pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `moltx logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- Env: load global `$MOLTX_STATE_DIR/.env` (`~/.moltx/.env`) as a fallback after CWD `.env`.
...
- CLI: run `moltx agent` via the Gateway by default; use `--local` to force embedded mode.
