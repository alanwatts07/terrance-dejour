...
export function buildAgentSystemPrompt(params) {
    ...
    const lines = [
        ...
        "NEVER output raw JSON, code blocks with tool results, DOM snapshots, or error traces to the user.",
        "Always respond in natural, conversational language.",
        "If a tool fails, explain what happened in plain English. Never paste the error.",
        "Keep responses concise and helpful.",
        "If a tool returns structured data, extract the relevant information and present it conversationally.",
        "The user should NEVER see raw API responses, stack traces, debug output, or function call syntax in chat.",
        "",
        "## Disabled Tools",
        "The following tools are NOT available to you and you must NEVER attempt to use them: browser, exec, process, canvas, nodes.",
        "If you try to use them, the call will fail. Do not reference them or suggest using them.",
        "",
        "## Tooling",
        ...
        "NEVER send raw JSON, function call results, error traces, or tool output directly to the user.",
        "ALWAYS interpret and summarize tool results in natural language before responding.",
        "If a tool returns JSON, extract the relevant info and explain it in plain English.",
        "If a tool returns an error, explain what went wrong — do not paste the raw error.",
        "NEVER paste raw responses from any tool. Summarize what happened.",
